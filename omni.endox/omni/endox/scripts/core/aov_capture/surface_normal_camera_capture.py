...
class ViewSurfaceNormalCapture:
    """Capture camera-space surface normals using Warp-accelerated GPU batch pipeline.

    Pipeline (per batch of ``write_interval`` frames):
        1. **Collect** - ``get_data(device=cuda)`` -> extract XYZ channels
           into f32 GPU buffer slot (async).
        2. **Rotate** - per-slot Warp kernel applies camera rotation + normalize
           in-place on the f32 GPU buffer (each frame may have a different
           rotation matrix, passed as a ``wp.array``).
        3. **Convert** - one Warp kernel converts ALL rotated f32 frames -> uint16
           BGR in parallel on the GPU.
        4. **Transfer + Write** - single ``.numpy()`` DMA of the whole
    ...
    # ---- Camera rotation helpers (CPU, kept for backward compat) -------------

    @staticmethod
    def get_camera_rotation_matrix(camera_prim):
        world_matrix = omni.usd.get_world_transform_matrix(camera_prim)
        rotation_matrix = world_matrix.ExtractRotationMatrix()
        return np.array(rotation_matrix)

    @staticmethod
    def transform_normals_to_camera_space(normals_world, camera_rot):
        normals_camera = np.tensordot(normals_world, camera_rot.T, axes=([2], [0]))
        norms = np.linalg.norm(normals_camera, axis=2, keepdims=True) + 1e-8
        return normals_camera / norms

    # ---- Warp kernels (f32 pipeline, with f16-input variants) ----------------

    # - Single-frame: f32 normals -> uint16 BGR --
    @staticmethod
    @wp.kernel
    ...
    # - Single-frame: f16 normals -> uint16 BGR --
    @staticmethod
    @wp.kernel
    ...
    # - Single-frame rotation: f32 in/out, rotation as wp.array --
    @staticmethod
    @wp.kernel
    def _rotate_normals_kernel(
        normals_in: wp.array(dtype=wp.float32, ndim=3),
        normals_out: wp.array(dtype=wp.float32, ndim=3),
        rot: wp.array(dtype=wp.float32, ndim=2),
    ):
        i, j = wp.tid()
        nx = normals_in[i, j, 0]
        ny = normals_in[i, j, 1]
        nz = normals_in[i, j, 2]
        rx = nx * rot[0, 0] + ny * rot[0, 1] + nz * rot[0, 2]
        ry = nx * rot[1, 0] + ny * rot[1, 1] + nz * rot[1, 2]
        rz = nx * rot[2, 0] + ny * rot[2, 1] + nz * rot[2, 2]
        length = wp.sqrt(rx * rx + ry * ry + rz * rz) + 1.0e-8
        normals_out[i, j, 0] = rx / length
        normals_out[i, j, 1] = ry / length
        normals_out[i, j, 2] = rz / length
    ...
    # - Batch rotation: in-place on f32 buffer, rotation as wp.array --
    @staticmethod
    @wp.kernel
    def _rotate_slot_kernel(
        ...
        rz = nx * rot[2, 0] + ny * rot[2, 1] + nz * rot[2, 2]
        length = wp.sqrt(rx * rx + ry * ry + rz * rz) + 1.0e-8
        buf[slot, i, j, 0] = rx / length
        buf[slot, i, j, 1] = ry / length
        buf[slot, i, j, 2] = rz / length
    ...
    # ---- GPU transform helper (for aov_capture_all.py) -----------------------

    @staticmethod
    def transform_normals_gpu(normals_cuda, camera_rot, device=None):
        """Rotate + normalize world-space normals on GPU -> f32 output.

        Handles both f16 and f32 annotator input.  The rotation matrix is
        uploaded as a ``wp.array`` (no scalar ``float`` params, avoiding the
        Warp 1.4.1 codegen bug).

        Returns
        -------
        wp.array
            ``(H, W, 3)`` float32 normals on CUDA.
        """
        dev = device or str(normals_cuda.device)
        h, w = normals_cuda.shape[0], normals_cuda.shape[1]

        # If annotator returned f16, upcast to f32 via a temp copy
        if normals_cuda.dtype == wp.float16:
            src_f32 = wp.empty((h, w, 3), dtype=wp.float32, device=dev)
            # Use a simple element-wise copy kernel would be ideal, but we
            # can reuse the slot copy kernel with a 4D wrapper:
            n_in = normals_cuda
            n_ch = n_in.shape[2] if n_in.ndim == 3 else 0
            # Quick inline upcast via numpy (small one-time cost for single frame)
            src_f32 = wp.array(
                normals_cuda.numpy().astype(np.float32),
                dtype=wp.float32, device=dev)
        else:
            src_f32 = normals_cuda
        ...
        wp.launch(
            kernel=ViewSurfaceNormalCapture._rotate_normals_kernel,
            dim=(h, w),
            inputs=[src_f32, out, rot_wp],
            device=dev,
        )
        return out
    ...
    def visualize_normals(normals, out_path, verbose=False):
        ...
        if isinstance(normals, wp.array) and "cuda" in str(normals.device):
            ...
            if normals.ndim != 3:
                ...
                ViewSurfaceNormalCapture._visualize_normals_numpy(
                    normals_np, out_path)
                return

            out_wp = wp.zeros((h, w, 3), dtype=wp.uint16, device=dev)
            kernel = (ViewSurfaceNormalCapture._normals_f32_to_uint16_kernel
                      if normals.dtype == wp.float32
                      else ViewSurfaceNormalCapture._normals_f16_to_uint16_kernel)
            wp.launch(kernel=kernel, dim=(h, w),
                      inputs=[normals, out_wp], device=dev)
        ...
        if verbose:
            ...
            print(f"  [normals_cam] {t_ms:.2f} ms  {out_path}")
    ...
    async def capture_and_visualize(self):
        ...
        print(f"[CameraNormals] Starting - "
        ...
        for timecode in range(start_tc, start_tc + self.num_timeline_frames):
            ...
            camera_rot = self.get_camera_rotation_matrix(self.camera_prim)
            batch_rots.append(camera_rot)
            ...
            if slot >= self.write_interval or is_last:
                self._flush_buffer(batch_paths, batch_rots, slot)
                batch_paths.clear()
                batch_rots.clear()
                slot = 0
    ...
    def _rotate_batch(self, batch_rots, count):
        h, w = self._raw_buf.shape[1], self._raw_buf.shape[2]
        for s in range(count):
            rot_np = batch_rots[s].astype(np.float32)
            rot_wp = wp.array(rot_np, dtype=wp.float32, device=self.device)
            wp.launch(
                kernel=ViewSurfaceNormalCapture._rotate_slot_kernel,
                dim=(h, w),
                inputs=[self._raw_buf, rot_wp, s],
                device=self.device,
            )
    ...
    def _flush_buffer(self, paths, batch_rots, count):
        if count == 0:
            return
        t0 = time.perf_counter()

        self._rotate_batch(batch_rots, count)
        ...
        if self.verbose:
            ...
            print(f"  [normals_cam] Flushed {count} frame(s): "
                  f"{first} -> {last}  ({t_ms:.2f} ms, {fmt})")


if __name__ == '__main__':
    camera_prim_path = "/World/CapsuleCam/Camera"
    output_dir = "C:/output/aov_capture/surface_normal_camera"
    normal_capturer = ViewSurfaceNormalCapture(
